...
#define B0_ASSERT(test, fmt, ...) \
    do \
    { \
        if (!(test)) \
        { \
            printk("B0: ASSERTION FAIL @ %s:%d\n", __FILE__, __LINE__); \
            printk("\t" fmt "\n", ##__VA_ARGS__); \
            (void)arch_irq_lock(); \
...
b0_segger_rtt_check_data_location_and_size(void)
{
    ...
    LOG_INF("B0: RTT data address: %p", __rtt_buff_data_start);
    LOG_INF("B0: RTT data size: 0x%zx", rtt_buff_size);
    B0_ASSERT(
    ...
    B0_ASSERT(0 == (rtt_buff_size % 0x1000), "RTT buffer size is not aligned to 4kB, size=0x%zx", rtt_buff_size);
